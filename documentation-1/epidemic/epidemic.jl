...
# ╔═╡ f164254c-744e-11ef-323a-0373f5298f06
begin
...
# ╔═╡ df410ada-3c81-4e91-8a24-11e979d14db3
md"# Categorical Epidemic Modelling
...
# ╔═╡ 0e2f1e0c-0084-4a14-8fe2-895306e84a50
md"
...
# ╔═╡ 5263adc9-8e75-4de1-a360-2c4a90734096
md"
...
# ╔═╡ 230ff88f-59b1-4c05-a729-b8db5d9d9c4b
begin	
...
# ╔═╡ fcabb8a9-996d-4c65-b786-4ef9c6e38c90
md"The tool graphviz can be used to visualize the petri-net created above."

# ╔═╡ 345e3a67-2de6-41bf-850f-b69241454cd4
md"
...
# ╔═╡ 817c605f-5279-47be-9e44-53e7bc784f0e
begin
...
# ╔═╡ c2d8084b-b54a-4d4f-a33a-488d80cbf413
md"
...
# ╔═╡ cf266e87-b179-4b42-985b-092939212374
begin
...
# ╔═╡ 0cb2a91c-16cc-4826-bd62-ac1de73957f9
...
# ╔═╡ 04398920-a9c2-4213-99e9-5e52cecb02b6
begin
	SVIEEvRD_composition_uwd = @relation (S, V, I, E, Ev, R, D) begin
	    unvax(S, E, I, R, D)
	    vax(V, Ev, R)
	    cross(S, I, V, Ev)
	end
...
# ╔═╡ b73f9702-d78c-44d3-96c1-dceaf2d2572b
md"We use Catlab's oapply to compose the models using our UWD. We use the SIERD\_opetri\_net for unvax, VEvR\_opetri\_net for vax, cross\_exposure\_opetri_net for cross.

The remarkable power in categorical modelling in composing various models has been demonstrated here."

# ╔═╡ dc50a69b-2f6d-47d5-8805-011d65666620
begin
	SVIEEvRD_opetri_net = oapply(
	    SVIEEvRD_composition_uwd,
	    Dict(
			:unvax => SEIRD_opetri_net, 
			:vax => VEvR_opetri_net, 
			:cross => cross_exposure_opetri_net
		) 
	)
...
# ╔═╡ 80b3e4b5-235f-4c9b-a0df-dc6f7d2da11a
md"""# Stratification of our Models
...
We explicitly extract the TISO componenents of the petri-net in order to associate them later to the various parts of our petri-nets which are to be typed. This association happens in the form of an C-Set Transformation.
...
# ╔═╡ b9fc93ad-126c-424e-a49a-6b9640a93cab
begin
	...
	m = to_graphviz(infectious_type)							# with names
	...
	infectious_type = map(infectious_type, Name=name->nothing)	# without names

	m
end

# ╔═╡ f7744802-18cd-4865-924a-14f248f22c47
md"""
### Hygiene Stratification
Now, we create a **hypothetical** "Hygiene" stratifictaion, associating a hygiene class to each individual. An individual can either begin to one of the two classes:
...
# ╔═╡ f104b330-0989-465b-8653-146cc67c9638
begin
	# Hygiene Stratification
	...
	hygiene_petri_net = LabelledPetriNet(
	    ...
	    :h => ((:H, :H) => (:H, :H)),   # interaction between two hygienic people
	    :uh => ((:U, :U) => (:U, :U)),  # interaction between two unhygienic people
	    :mh => ((:U, :H) => (:U, :H)),  # interaction between one from each kind
...
# ╔═╡ 8b83d64b-8b84-4dba-bd81-59b4c3160110
md"""Written below is a helper function to add identity transitions for typing with state transitions in our pre-made SVIEEvRD model"""

# ╔═╡ de52da4e-a5ad-435c-baf3-ead7d2c05e5f
begin
...
typed_SVIEEvRD_petri_net = ACSetTransformation(SVIEEvRD_petri_net, infectious_type,
    ...
    T = vcat(t_interact, t_status, t_status, t_status, t_status, t_interact, t_status, repeat([t_status], ns(SVIEEvRD_petri_net))),
    I = vcat(i_interact1, i_interact2, i_status, i_status, i_status, i_status, i_interact1, i_interact2, i_status, repeat([i_status], ns(SVIEEvRD_petri_net))),
    O = vcat(o_interact1, o_interact2, o_status, o_status, o_status, o_status, o_interact1, o_interact2, o_status, repeat([o_status], ns(SVIEEvRD_petri_net))),
    Name = name -> nothing
)

# ╔═╡ c00bed06-739a-47c3-a7d8-d3a1db080a12
md"""We must also ensure that our transformation is a natural transformation to ensure commutativity of all morphisms. This fortunately serves as a useful tool to check for proper specification of the typing."""

# ╔═╡ 8b22478e-208e-4d51-b5a0-019737444ede
@assert is_natural(typed_SVIEEvRD_petri_net)

# ╔═╡ e357305f-964c-4bc8-a72a-61b098f86566
md"""We can once again use graphviz to visualize the typing, that is, the transformation that we are constructing."""

# ╔═╡ aed70267-61d6-4697-b7e9-2b80d3b1be62
to_graphviz(typed_SVIEEvRD_petri_net, program="dot")

# ╔═╡ 609598d0-4474-4126-85a5-8f2f5e9f0a9d
md"""Finally, we use a pullback to join together the stratification scheme with our SVIEEvRD model, joined together along the typing scheme, and we extract the object of the pullback which will be our desired petri-net.
...
# ╔═╡ b3876ba0-f159-4a8a-9fed-aabab44e643e
begin
	# Stratify the model
	UH_SVIEEvRD_petri_net =  ob(pullback(typed_hygiene_petri_net, typed_SVIEEvRD_petri_net))

	# Flatten the labels, since our ODE can't handle tuple "names"
	UH_SVIEEvRD_petri_net = flatten_labels(UH_SVIEEvRD_petri_net)
	to_graphviz(UH_SVIEEvRD_petri_net)
end

# ╔═╡ 8d19de55-b39d-4515-8113-8dd129e8b67b
md"# Specifying the Parameters of the System
...
# ╔═╡ 0830f2ec-71df-4301-a7cb-0c56fed27d7a
begin
	p = LVector(
...
# ╔═╡ 1d6586be-0dc5-425b-8199-c37050e1f231
md"Finally, we generate a system of ordinary differential equations (in their associated vectorfield) using the vectorfield function, and use julia's DifferentialEquations's ODEProblem to solve it."

# ╔═╡ 5fb94293-4d8a-4aa8-900e-1a1d1abeaa40
begin
	soln = solve(ODEProblem(vectorfield(UH_SVIEEvRD_petri_net), u0, (0.0, 200.0), p))
...
manifest_format = "2.0"
project_hash = "35af612c9b27179563186f968e22d5de559a7518"
...
git-tree-sha1 = "9d026482e58c7152645670034e6ae84a26b7624b"
...
deps = ["Catlab", "Compose", "LabelledArrays", "LinearAlgebra", "LinearMaps", "Plots", "RecipesBase", "RecursiveArrayTools", "Reexport", "StaticArrays"]
git-tree-sha1 = "abef35986e3a945904ffe790fc362c1e8d1c6fff"
uuid = "5fd6ff03-a254-427e-8840-ba658f502e32"
version = "0.2.2"
weakdeps = ["AlgebraicPetri", "DelayDiffEq", "OrdinaryDiffEq"]

    [deps.AlgebraicDynamics.extensions]
...
git-tree-sha1 = "28cca88bde1068ccf57f950e639df67975517c25"
...
git-tree-sha1 = "f7eb28a9f20bb313a0ec57a27e34344529daafb2"
...
git-tree-sha1 = "323bdb465a125483fa6c1b4fbed52f0be16ac0fe"
